...
This package relies on @nicolawealth/ioc as a peer dependency. It will not be
bundled with rate_limit, so you need to install it in your project:
...
Creates a wrapper that ensures fn() runs at most once every
delayBetweenCallsMs milliseconds.
...
Wraps an async function so it runs at most once per delay window, but always
processes the latest parameters.
